...
export class RandomizerService {
  ...
  public randomNumber = (max: number): number => Math.floor(Math.random() * max) + 1;

  // simulates a coin flip
  public coinFlip = (): string => this.randomNumber(2) === 1 ? 'Tails' : 'Heads';
  ...
  public multi = (num: number, func: any): any[] => {
    const results: any[] = [];
    for (let i = 0; i < num; i++) {
      results.push(func());
    }
